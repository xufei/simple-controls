...
DataGrid.prototype = {
	...
	sort: function(key, order, comparer) {
...
DataRow.prototype = {
	create: function () {
		...
		for (var i = 0; i < this.grid.columns.length; i++) {
			...
			if (column.labelFunction) {
				cell.innerHTML = column.labelFunction(this.data);
			}
			else {
				cell.innerHTML = this.data[column.field] || "";
			}
	...
	set: function (key, value) {
		...
		for (var i = 0; i < this.grid.columns.length; i++) {
			if (this.grid.columns[i].field === key) {
				var column = this.grid.columns[i];
				if (column.labelFunction) {
					this.dom.childNodes[i].innerHTML = column.labelFunction(this.data);
				}
				else {
					this.dom.childNodes[i].innerHTML = this.data[key] || "";
				}
				break;
			}
		}
	...
	refresh: function (data) {
		...
		for (var i = 0; i < this.grid.columns.length; i++) {
			...
			this.set(column.keyField, data[column.keyField]);
		}
	}
}.extend(EventDispatcher);
